...
process_exit (void)
{
  ...
  //TODO
  //munmap for all thread's mmap descriptiors
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  if (t->pagedir == NULL) // add check for page table 
    goto done;
              ...
              uint32_t read_bytes, uint32_t zero_bytes, bool writable) 
{
  ...
  while (read_bytes > 0 || zero_bytes > 0) {
    ...
    size_t page_zero_bytes = PGSIZE - page_read_bytes;
      ...
      // if you can evict then 
      // try to do palloc get page again
      // panic the kernel
      // create frame 
      //TWO INSTANCES of install_page

      //frame struct get the kpage 
      //originally returning the frame pointer
      //kpage = frame paddress

      //WRITE A FUNCTION TO FIND PAGES BY THEIR ADDRESS

      //replace all the palloc get page and free pages to create and free frame
      //create sup pg tb entry for the user page and associate with the frame you just created
      //
